...
const useFolder = (
  ...
  const mongoCursorRef = useRef<
    { afterId: string; afterLabel: string } | undefined
  >(undefined);
  const mongoFsRef = useRef<MongoDBFileSystem | undefined>(undefined);
    ...
    async (newFile?: string, oldFile?: string) => {
      if (oldFile) {
        ...
        try {
          ...
          mongoFsRef.current =
            mountedFs instanceof MongoDBFileSystem ? mountedFs : undefined;
          ...
          if (mongoFsRef.current) {
            ...
            mongoCursorRef.current = result.nextCursor ?? undefined;
