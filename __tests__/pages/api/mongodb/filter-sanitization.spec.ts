import { buildMongoTagsFilter, sanitizeFilter } from "utils/mongoApi";
...
describe("sanitizeFilter", () => {
  it("allows safe operators", () => {
    ...
    expect(() =>
      sanitizeFilter({ name: { $options: "i", $regex: "test" } })
    ).not.toThrow();
  ...
  it("rejects $where operator", () => {
    expect(() => sanitizeFilter({ $where: "this.a > 1" })).toThrow(
      "Disallowed filter operator: $where"
    );
  });

  it("rejects $expr operator", () => {
    expect(() => sanitizeFilter({ $expr: { $gt: ["$a", "$b"] } })).toThrow(
      "Disallowed filter operator: $expr"
    );
  });

  it("rejects $function operator", () => {
    expect(() =>
      sanitizeFilter({ $function: { body: "return true" } })
    ).toThrow("Disallowed filter operator: $function");
  });

  it("rejects nested dangerous operators", () => {
    expect(() => sanitizeFilter({ $and: [{ $where: "true" }] })).toThrow(
      "Disallowed filter operator: $where"
    );
  });
  ...
  it("allows plain field filters", () => {
    expect(() =>
      sanitizeFilter({ category: "fruit", name: "test" })
    ).not.toThrow();
  });

  it("builds safe include/exclude tag filters", () => {
    const filter = buildMongoTagsFilter({
      exclude: ["tilbud", " tilbud "],
      include: ["Børnemad", "Kolonial"],
    });

    expect(filter).toEqual({
      $and: [
        { tags: { $in: ["Børnemad", "Kolonial"] } },
        { tags: { $nin: ["tilbud"] } },
      ],
    });
    expect(() => sanitizeFilter(filter)).not.toThrow();
  });
